...
class HttpControllers (private val repository: ArticleRepository) {
    ...
    @GetMapping("/")
    fun findAll() : List<Article> = repository.findAllByOrderByAddedAtDesc()

    @GetMapping("/{slug}")
    fun findOne(@PathVariable slug: String) : Article? =  repository.findBySlug(slug) ?: throw ResponseStatusException(HttpStatus.NOT_FOUND, "This article does not exist")
